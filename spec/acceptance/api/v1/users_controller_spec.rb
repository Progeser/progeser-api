...
resource 'Users' do
  ...
  post '/api/v1/users/:invitation_token/create_from_invite' do
    ...
    example 'Create a user from an invite and destroy it' do
      ...
      expect(response['email']).to eq(invite.email)
      expect(response['role']).to eq(invite.role)
      expect(response['first_name']).to eq(invite.first_name)
      expect(response['last_name']).to eq(invite.last_name)
      expect(response['laboratory']).to eq(invite.laboratory)
  ...
  post '/api/v1/users/:creation_token/create_from_account_request' do
    ...
    example 'Create a user from an account_request and destroy it' do
      ...
      response = JSON.parse(response_body)
      expect(response['email']).to eq(account_request.email)
      expect(response['role']).to eq('requester')
      expect(response['first_name']).to eq(account_request.first_name)
      expect(response['last_name']).to eq(account_request.last_name)
